...
ShaderProgram::ShaderProgram(OpenGLContext *context)
    : vertShader(), fragShader(), prog(),
      ...
      attrPos(-1), attrNor(-1), attrCol(-1), attrUV(-1), attrFlag(-1),
      unifModel(-1), unifModelInvTr(-1), unifViewProj(-1), unifColor(-1), unifSampler2D(-1), unifSampler(-1), unifTime(-1),
      context(context)
{}
...
void ShaderProgram::setSampler(int i){
    useMe();
    if(unifSampler != -1){
        context->glUniform1i(unifSampler, i);
...
void ShaderProgram::drawInterleaved(Drawable &d, bool drawTransparent)
{
    useMe();
    ...
    if(!drawTransparent){
        ...
        if(attrFlag != -1) {
            context->glEnableVertexAttribArray(attrFlag);
            context->glVertexAttribPointer(attrFlag, 1, GL_FLOAT, false, 3 * sizeof(glm::vec4), (void*)(2 * sizeof(glm::vec4) + sizeof(glm::vec2)));
            context->printGLErrorLog();
        }
        ...
        if(d.elemCountTransparent() < 0) {
            throw std::out_of_range("Attempting to draw a transparent drawable with m_transparentCount of " + std::to_string(d.elemCount()) + "!");
        }
...
void ShaderProgram::drawUI(Drawable &d){
        useMe();
        if(d.elemCount() < 0) {
            throw std::out_of_range("Attempting to draw a drawable with m_count of " + std::to_string(d.elemCount()) + "!");
        }
        ...
        if(attrPos != -1) {
            context->glEnableVertexAttribArray(attrPos);
            context->glVertexAttribPointer(attrPos, 2, GL_FLOAT, false, 2*  sizeof(glm::vec2), (void*)0);
        ...
        if(attrUV != -1) {
            context->glEnableVertexAttribArray(attrUV);
            context->glVertexAttribPointer(attrUV, 2, GL_FLOAT, false, 2* sizeof(glm::vec2), (void*)(sizeof(glm::vec2)));
            context->printGLErrorLog();
        }
...
void ShaderProgram::setupMemberVars()
{
    ...
    attrFlag = context->glGetAttribLocation(prog, "vs_Flag");
