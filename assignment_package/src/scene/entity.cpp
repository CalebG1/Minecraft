...
void Entity::rotateToAngles(float hor, float ver) {
    // First undo current rotations
    rotateOnUpLocal(-horRotation);
    rotateOnRightLocal(-verRotation);
    // Apply new rotations
    rotateOnRightLocal(ver);
    rotateOnUpLocal(hor);
}
...
// Modifed from the original so that the up vector isn't affected with L/R rotation
// This is used for desired camera/flight mode behavior
